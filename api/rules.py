...
Implementa las 7 reglas del PROMPT_MAESTRO_MAG.md usando pandas
"""
...
# ── REGLA 6: MYSTATUS ────────────────────────────────────────────
def calcular_mystatus(status_recibo: str) -> str:
    """Calcula el estado interno de una póliza basado en el status del recibo."""
    mapping = {
...
def normalizar_poliza(num: str) -> str:
    ...
    normalizado = re.sub(r'^0+(\d)', r'\1', num.strip())
    return normalizado
...
def calcular_kpis_polizas(polizas: list, anio: int) -> dict:
    ...
    return {
        ...
        "canceladas_vida": sum(1 for p in del_anio if p.get("ramo_codigo") == 11 and p.get("status_recibo") != "PAGADA"),
        "canceladas_gmm": sum(1 for p in del_anio if p.get("ramo_codigo") == 34 and p.get("status_recibo") != "PAGADA"),
